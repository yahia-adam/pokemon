...
int **setMap(int col, int line)
{
    ...
    //
    float minHerbe = 0.2, maxHerbe = 0.45, maxObst = 0.8;
    for(int lineInd = 0; lineInd < line; lineInd++)
    {

        map[lineInd]=malloc(sizeof(int **)*col);
        for(int colInd = 0; colInd < col; colInd++)
        {
            float perlin = perlin2d(colInd, lineInd, freq, 10);
